//! Editor deferred viewport rendering. Split from editor_win.rs; behavior unchanged.
        ...
        let title = {
            ...
            ed.tabs
                ...
                .map(|t| match crate::i18n::current() {
                    crate::i18n::Lang::Zh => {
                        format!("iShell 编辑器 — {}·{}", t.server, t.editor.filename())
                    }
                    crate::i18n::Lang::En => {
                        format!("iShell Editor — {}·{}", t.server, t.editor.filename())
                    }
                })
                                ...
                                |ui| {
                                        ...
                                        let save = match (t.save_at, t.save_done_at) {
                                            (Some(t0), None) => {
                                                ...
                                                let actual = if t.save_total > 0 {
                                                    (t.save_done as f64 / t.save_total as f64)
                                                        .clamp(0.0, 1.0)
                                                } else if !t.is_saving() {
                                                    1.0
                                                } else {
                                                ...
                                                let g = actual
                                                    .min(((now - t0) / MIN_SWEEP).clamp(0.0, 1.0));
                                    ...
                                    let labels: Vec<(u64, String, String, f32, f32)> = ed
                                        ...
                                        .map(|(i, t)| {
                                            ...
                                            let prog = if t.load_id.is_some() {
                                                (t.load_done as f32 / t.load_total.max(1) as f32)
                                                    .clamp(0.0, 1.0)
                                            } else {
                                                -1.0
                                            };
                                    ...
                                    if let Some((from, to)) = reord {
                                        if from < ed.tabs.len() && to < ed.tabs.len() {
                                            ...
                                            ed.active = if ed.active == from {
                                                to
                                            } else if ed.active == to {
                                                from
                                            } else {
                                                ed.active
                                            };
            ...
            egui::CentralPanel::default()
                ...
                .show(vctx, |ui| {
                    ...
                    if let Some(tab) = ed.tabs.get_mut(active) {
                        ...
                        if tab.is_conflict() {
                            egui::Frame::new()
                                .fill(egui::Color32::from_rgb(255, 244, 220))
                                    ...
                                    ui.horizontal(|ui| {
                                        ui.label(
                                            RichText::new(format!(
                                                ...
                                                crate::i18n::tr(
                                                    "文件已被外部修改，未保存",
                                                    "File changed externally; not saved"
                                                )
                                            ))
                                            .color(Palette::TEXT)
                                            .size(12.0),
                                        ...
                                        ui.with_layout(
                                            ...
                                            |ui| {
                                                if ui
                                                    .button(crate::i18n::tr("取消", "Cancel"))
                                                    .clicked()
                                                {
                                                    tab.save = super::SaveState::Idle;
                                                    // 取消冲突横幅，回到空闲（保留 dirty）
                                                }
                                                if ui
                                                    .add(
                                                        egui::Button::new(
            ...
            if let Some(ti) = ed.close_tab_confirm {
                ...
                egui::Modal::new(egui::Id::new("editor_tab_close_modal")).show(vctx, |ui| {
                    ...
                    ui.vertical_centered(|ui| {
                        ui.label(
                            RichText::new(crate::i18n::tr("关闭标签", "Close tab"))
                                .size(16.0)
                                .strong(),
                        );
                        ui.add_space(6.0);
                    ...
                    ui.horizontal(|ui| {
                        ...
                        if ui
                            .add(
                                egui::Button::new(
                                    RichText::new(crate::i18n::tr("保存并关闭", "Save & close"))
                                        .color(egui::Color32::WHITE),
                                )
                                .fill(Palette::ACCENT)
                        ...
                        if ui
                            .add(
                                egui::Button::new(
                                    RichText::new(crate::i18n::tr("不保存", "Discard"))
