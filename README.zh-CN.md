...
| 指标 | 数值 |
|---|---|
| 二进制 | 单文件，无运行时依赖/守护进程 —— **Linux ~13 MB · macOS ~10 MB · Windows ~12 MB**（体积优化：opt-level `s` + fat LTO + strip） |
...
- **认证**：密码、私钥文件，或 **SSH Agent**（`SSH_AUTH_SOCK` / Windows OpenSSH 命名管道）
...
**文件与传输**
- SFTP：树形目录 + 列表、**名称过滤**、**点击表头按名称/大小/时间排序**（大小、时间首次点击为降序）、拖拽上传、**「在终端打开此目录」**、改权限 / 重命名 / 删除 / 复制路径、可选默认下载目录
