...
| 指标 | 数值 |
|---|---|
| 二进制 | 单文件，无运行时依赖/守护进程 —— **Linux ~23 MB · macOS ~14–16 MB · Windows ~20 MB** |
...
**文件与监控**
- SFTP：树形目录 + 列表、**点击表头按名称/大小/时间排序**（大小、时间首次点击为降序——先看最大/最新）、拖拽上传、下载（含**文件夹递归**）、改权限 / 重命名 / 删除 / 复制路径、可选默认下载目录
- **多文件并发传输**（同一服务器最多 6 个，不同服务器互不影响）、**可中途取消**、失败可查看原因
- **多标签文本编辑器**（独立 OS 窗口）：**行号**、语法高亮、查找替换、大文件只读虚拟化（可切换为可编辑）
- **超轻量看图工具**（独立 OS 窗口）：双击 `png / jpg / gif / bmp` 打开——滚轮缩放、拖动平移、适应/1:1、另存为；纯 Rust 解码器，几乎不增体积
