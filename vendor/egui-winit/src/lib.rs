    ...
    fn on_ime(&mut self, ime: &winit::event::Ime) {
        ...
        match ime {
            ...
            winit::event::Ime::Preedit(text, None) => {
                // ─── iShell patch（见 ISHELL_PATCH.md）───────────────────────────
                // 上游把「没带光标位置」的组字事件里的 text 整个丢掉（原本绑成 `_`），
                // 并当作组字结束推送 ImeEvent::Disabled。但 X11/fcitx 在组字**进行中**
                // 按下 Shift 等修饰键时，发的正是 `Preedit("已输入的拼音", None)`：
                // 组字并没有结束，只是这一拍不报光标位置。于是应用侧收到 Disabled 就把
                // 已输入的拼音从输入框里抹掉，表现为「按 Shift 后拼音不显示了，但其实还
                // 在输入法的组字缓冲里；再敲/删一个字符，下一拍带光标的 Preedit 把完整串
                // 补回来，之前的字符又全冒出来」。
                // 改为：text 非空 => 组字仍在进行，照常转发；仅 text 为空才当组字结束。
                if !text.is_empty() {
                    self.ime_event_enable();
                    self.egui_input
                        .events
