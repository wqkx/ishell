...
impl ContextInner {
    ...
    pub(crate) fn is_view_current(&self, view: &MainThreadBound<Retained<NSView>>) -> bool {
        run_on_main(|mtm| {
            // iShell 本地补丁：窗口最小化时 NSOpenGLContext 没有当前视图（view 返回 None），
            // 上游此处 .expect 会直接 panic（macOS 多窗口最小化到 Stage Manager 时闪退）。
            // 改为「无视图时返回 false」——make_current 据此正常重新设定视图，不再崩溃。
            match self.raw.view(mtm) {
                Some(v) => v == *view.get(mtm),
                None => false,
            }
        })
    }
