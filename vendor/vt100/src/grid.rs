...
impl Grid {
    ...
    pub fn scroll_up(&mut self, count: u16) {
        for _ in 0..(count.min(self.size.rows - self.scroll_top)) {
            ...
            // A top-anchored scrolling region still scrolls rows off the top
            // of the terminal, so real terminal emulators preserve those rows
            // in scrollback. Inline TUIs (for example Codex/ratatui) rely on
            // this to insert completed output above their live viewport.
