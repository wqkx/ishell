...
多机共用一台 AI 服务器时：开启 AI 控制后，iShell 会在连上后的空闲提示符处自动把
ISHELL_MCP_TOKEN 注入本会话，请求据此精确回到发起的那台电脑，不会串到别人的窗口。
注入要求「连上后还没动过键盘」（避免把 token 打进某个正在等输入的密码提示符）——
连上就立刻打字的话本次不注入，退回原有的「多实例弹窗选择」。也可在设置里用
「复制配对配置」手工写进 MCP 环境变量，那条路不受此限制。
详见 README「AI / MCP 集成」。
EOF
